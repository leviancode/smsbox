...
fun TemplateWithRecipients.toDomainTemplate() = Template(
    ...
    recipientGroup = recipients?.group?.toDomainRecipientGroup() ?: RecipientGroup()
)


fun Template.toDataTemplate() = TemplateData(
    templateId = id,
...
fun Template.toDataTemplate(recipientGroupId: Int) = TemplateData(
    templateId = id,
    templateGroupId = groupId,
...
fun List<TemplateWithRecipients>.toDomainTemplatesWithRecipients() = map { it.toDomainTemplate() }
fun List<Template>.toDataTemplates() = map { it.toDataTemplate() }